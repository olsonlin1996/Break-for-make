...
def main(args):
    ...
    def freeze_linear_params(layer, weight_indices, row_or_column, bias_indices=None, weight_hook_handle=None, bias_hook_handle=None):
        # https://github.com/galidor/PyTorchPartialLayerFreezing
        ...
        if row_or_column == 'row' and max(weight_indices) >= layer.weight.shape[0]:
            print(max(weight_indices), layer.weight.shape[0])
            raise IndexError("row_weight_indices must be less than the number output channels")
        if row_or_column == 'column' and max(weight_indices) >= layer.weight.shape[1]:
            print(max(weight_indices), layer.weight.shape[1])
            raise IndexError("column_weight_indices must be less than the number output channels")
        ...
        if row_or_column == 'column':
            weight_multiplier = torch.ones(layer.weight.shape[1]).to(accelerator.device)
        elif row_or_column == 'row':
            weight_multiplier = torch.ones(layer.weight.shape[0]).to(accelerator.device)
    ...
    for attn_processor_name, attn_processor in unet.attn_processors.items():
        ...
        attn_module.to_q.set_lora_layer(
            LoRALinearLayer(
                in_features=attn_module.to_q.in_features, out_features=attn_module.to_q.out_features, rank=args.rank, orth=True
            )
        )
        attn_module.to_k.set_lora_layer(
            LoRALinearLayer(
                in_features=attn_module.to_k.in_features, out_features=attn_module.to_k.out_features, rank=args.rank, orth=True
            )
        )
        attn_module.to_v.set_lora_layer(
            LoRALinearLayer(
                in_features=attn_module.to_v.in_features, out_features=attn_module.to_v.out_features, rank=args.rank, orth=True
            )
        )
        ...
        # indices_row = list(range(in_features//2))
        # indices_column = list(range(in_features//2))
        freeze_linear_params(attn_module.to_q.lora_layer.up, list(range(attn_module.to_q.lora_layer.up.weight.shape[0]//2)), row_or_column='row')
        freeze_linear_params(attn_module.to_k.lora_layer.up, list(range(attn_module.to_k.lora_layer.up.weight.shape[0]//2)), row_or_column='row')
        freeze_linear_params(attn_module.to_v.lora_layer.up, list(range(attn_module.to_v.lora_layer.up.weight.shape[0]//2)), row_or_column='row')
        freeze_linear_params(attn_module.to_out[0].lora_layer.up, list(range(attn_module.to_out[0].lora_layer.up.weight.shape[0]//2)), row_or_column='row')

        freeze_linear_params(attn_module.to_q.lora_layer.down, list(range(attn_module.to_q.lora_layer.down.weight.shape[1]//2)), row_or_column='column')
        freeze_linear_params(attn_module.to_k.lora_layer.down, list(range(attn_module.to_k.lora_layer.down.weight.shape[1]//2)), row_or_column='column')
        freeze_linear_params(attn_module.to_v.lora_layer.down, list(range(attn_module.to_v.lora_layer.down.weight.shape[1]//2)), row_or_column='column')
        freeze_linear_params(attn_module.to_out[0].lora_layer.down, list(range(attn_module.to_out[0].lora_layer.down.weight.shape[1]//2)), row_or_column='column')
        ...
        unet_lora_parameters.extend(attn_module.to_q.lora_layer.up.parameters())
        unet_lora_parameters.extend(attn_module.to_k.lora_layer.up.parameters())
        unet_lora_parameters.extend(attn_module.to_v.lora_layer.up.parameters())
        unet_lora_parameters.extend(attn_module.to_out[0].lora_layer.up.parameters())

        unet_lora_parameters_2.extend(attn_module.to_q.lora_layer.down.parameters())
        unet_lora_parameters_2.extend(attn_module.to_k.lora_layer.down.parameters())
        unet_lora_parameters_2.extend(attn_module.to_v.lora_layer.down.parameters())
        unet_lora_parameters_2.extend(attn_module.to_out[0].lora_layer.down.parameters())
    ...
    def load_model_hook(models, input_dir):
        ...
        # lora_state_dict, network_alphas = LoraLoaderMixin.lora_state_dict(args.resume_lora_weights)
...
#    )
    ...
    if not args.train_text_encoder:
        ...
        if args.multi_style:
            ...
            prompt_embeds_3 = instance_prompt_hidden_states_3
            unet_add_text_embeds_3 = instance_pooled_prompt_embeds_3
            prompt_embeds_4 = instance_prompt_hidden_states_4
            unet_add_text_embeds_4 = instance_pooled_prompt_embeds_4
        else:
            prompt_embeds_2 = instance_prompt_hidden_states_2
            unet_add_text_embeds_2 = instance_pooled_prompt_embeds_2
    ...
    if args.multi_style:
        train_dataset_2 = DreamBoothDataset(
            instance_data_root=args.instance_data_dir_2,
            class_data_root=args.class_data_dir if args.with_prior_preservation else None,
            class_num=args.num_class_images,
            size=args.resolution,
            center_crop=args.center_crop,
        )
        train_dataset_3 = DreamBoothDataset(
            instance_data_root=args.instance_data_dir_3,
            class_data_root=args.class_data_dir if args.with_prior_preservation else None,
            class_num=args.num_class_images,
            size=args.resolution,
            center_crop=args.center_crop,
        )
        train_dataset_4 = DreamBoothDataset(
            instance_data_root=args.instance_data_dir_4,
            class_data_root=args.class_data_dir if args.with_prior_preservation else None,
            class_num=args.num_class_images,
            size=args.resolution,
            center_crop=args.center_crop,
    ...
    if args.train_text_encoder:
        ...
        # )
        if args.multi_style:
            unet, optimizer, optimizer_2, train_dataloader, train_dataloader_2, train_dataloader_3, train_dataloader_4, lr_scheduler, lr_scheduler_2= accelerator.prepare(
                unet, optimizer, optimizer_2, train_dataloader, train_dataloader_2, train_dataloader_3, train_dataloader_4, lr_scheduler, lr_scheduler_2
            )
        else:
            unet, optimizer, optimizer_2, train_dataloader, train_dataloader_2, lr_scheduler, lr_scheduler_2= accelerator.prepare(
                unet, optimizer, optimizer_2, train_dataloader, train_dataloader_2, lr_scheduler, lr_scheduler_2
            )
...
#    for epoch in range(first_epoch, 1500):
        ...
        for step, batches in loader_iter:
                ...
                if not args.train_text_encoder:
                    if args.multi_style:
                        unet_added_conditions_2 = {
                                "time_ids": add_time_ids.repeat(elems_to_repeat_2, 1),
                                "text_embeds": unet_add_text_embeds_2.repeat(elems_to_repeat_2, 1),
                        }
                        prompt_embeds_input_2 = prompt_embeds_2.repeat(elems_to_repeat_2, 1, 1)
                        model_pred_2 = unet(
                                noisy_model_input_2,
                                timesteps,
                        ...
                        unet_added_conditions_3 = {
                                "time_ids": add_time_ids.repeat(elems_to_repeat_2, 1),
                                "text_embeds": unet_add_text_embeds_3.repeat(elems_to_repeat_2, 1),
                        }
                        prompt_embeds_input_3 = prompt_embeds_3.repeat(elems_to_repeat_2, 1, 1)
                        model_pred_3 = unet(
                                noisy_model_input_3,
                                timesteps,
                                prompt_embeds_input_3,
                                added_cond_kwargs=unet_added_conditions_3,
                        ).sample

                        unet_added_conditions_4 = {
                                "time_ids": add_time_ids.repeat(elems_to_repeat_2, 1),
                                "text_embeds": unet_add_text_embeds_4.repeat(elems_to_repeat_2, 1),
                        }
                        prompt_embeds_input_4 = prompt_embeds_4.repeat(elems_to_repeat_2, 1, 1)
                        model_pred_4= unet(
                                noisy_model_input_4,
                                timesteps,
                                prompt_embeds_input_4,
                                added_cond_kwargs=unet_added_conditions_4,
                        ).sample
                    else:
                        unet_added_conditions_2 = {
                                "time_ids": add_time_ids.repeat(elems_to_repeat_2, 1),
            ...
            logs = {"loss": loss_1.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
